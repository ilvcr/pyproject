...
'''
 ...
 Last modified: 2018-03-26 14:27
 Email        : liyaoliu@foxmail.com or ilvcr@outlook.com
 Filename     : cnnInTensorflow.py
 Description  :
'''
...
#初始化第二个卷积层的权值和偏置
W_conv2 = weight_variable([5, 5, 32, 64])#5*5的采样窗口，64个卷积核从一个平面抽取特征
...
h_fc1_drop = tf.nn.dropoot(h_fc1, keep_prob)
...
#结果存放在一个布尔列表中
#argmax返回一维张量最大值所在的位置
correct_prediction = tf.equal(tf.argmax(prediction), tf.argmax(y, 1))
#求准确率
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

with tf.Session() as sess:
    sess.run(tf.global_variale_initializer())
