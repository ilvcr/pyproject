...
def upload_thread(fileName, clientInfo):
	...
	# 回复客户端上传请求
	clientSocket.sendto(sendDataFirst, clientInfo)  #第一次用随机端口发送

	while True:
		# 接收客户端发送的数据
		responseData = clientSocket.recvfrom(1024) #第二次客户连接我随机端口
		...
		if packetOpt[0] == 3 and packetNum[0] == fileNum:
			...
			# 回复客户端ACK信号
			clientSocket.sendto(sendData, clientInfo) #第二次用随机端口发
...
def download_thread(fileName, clientInfo):
	...
	try:
		...
		# 发送错误信息
		clientSocket.sendto(errorData, clientInfo)  #文件不存在时发送
	...
	while True:
		...
		# 向客户端发送文件数据
		clientSocket.sendto(sendData, clientInfo)  #数据第一次发送
		...
		# 第二次接收数据
		responseData = clientSocket.recvfrom(1024)
...
def main():
	...
	while True:
		# 接收客户端发送的消息
		recvData, clientInfo = clientSocket.recvfrom(1024)  #　第一次客户连接69端口
