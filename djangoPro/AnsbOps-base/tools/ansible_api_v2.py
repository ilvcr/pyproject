...
def run_ansible(module_name,module_args,host_list,port,passwd):
    """ansible api"""
    ...
    options = Options(connection='ssh', module_path=None, forks=5, become=True,
                      ...
                      become_user='root', remote_user="root", check=False, diff=False
                      )
    ...
    for ip in host_list:
        inventory.add_host(host=ip, port=int(port))
    ...
    for ip in host_list:
        ...
        variable_manager.set_host_variable(host=host, varname='ansible_ssh_pass', value=passwd)
...
if __name__ == "__main__":
    ...
    ret = run_ansible(module_name,module_args,host_list)
    print(ret)